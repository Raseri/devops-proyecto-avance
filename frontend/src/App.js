...
function renderError(message) {
  const app = document.querySelector('#app');
  app.innerHTML = `
    <div style="padding: 40px; text-align: center; color: #111; background: #fff; min-height: 100vh; display: flex; align-items: center; justify-content: center; flex-direction: column;">
      <h2 style="color: #c62828; margin-bottom: 16px;">Error al cargar la aplicación</h2>
      <p style="color: #666; margin-bottom: 24px;">${message}</p>
      <button onclick="location.reload()" style="padding: 12px 24px; background: #1a1a1a; color: white; border: none; border-radius: 8px; cursor: pointer; font-size: 16px;">
        Recargar
      </button>
    </div>
  `;
